...
def classify_node(rel_path: str, content_roots: tuple[str, ...]) -> str:
...
def resolve_internal_link(from_path: pathlib.Path, raw_target: str, root_dir: pathlib.Path) -> str | None:
...
def extract_links(text: str, from_path: pathlib.Path, root_dir: pathlib.Path) -> list[str]:
    pattern = re.compile(r'\[[^\]]*\]\(([^)]+)\)')
    links: list[str] = []
    for match in pattern.finditer(text):
        target = resolve_internal_link(from_path, match.group(1).strip(), root_dir)
        if target:
            links.append(target)
    return links
...
<body>
    ...
    <script>
        ...
        function layoutComponent(component) {
            ...
            const radius = Math.max(150, Math.sqrt(component.length) * 28);
            ...
            const insightRadius = radius * 2.2;
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Generate a knowledge graph from markdown files.')
    parser.add_argument('--root', type=pathlib.Path, default=ROOT_DIR, help='Directory that contains the knowledge base.')
    parser.add_argument('--output', type=pathlib.Path, default=None, help='Output HTML file path.')
    args = parser.parse_args()

    root_dir = args.root.resolve()
    output_file = args.output.resolve() if args.output else root_dir / 'graph.html'
    ...
    for file_path in files:
        ...
        if not any(rel_path == root or rel_path.startswith(f'{root}/') for root in content_roots):
            continue
        if file_path.name == 'README.md':
            continue

        raw_text = file_path.read_text(encoding='utf-8')
        kind = classify_node(rel_path, content_roots)
        ...
        links = extract_links(raw_text, file_path, root_dir)
        node = {
            'id': rel_path,
            'rel_path': rel_path,
            'kind': kind,
